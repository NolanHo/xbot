...
- **Background interactive SubAgent completion MUST append final assistant reply to `placeholder.messages`.** The foreground path appends `llm.NewAssistantMessage(out.Content)` + persists to DB, but the background goroutine path only stored `lastReply = out.Content` without adding it to messages. `GetAgentSessionDump` reads `ia.messages` — without the assistant message, the session view shows only user message + tool calls but no final reply. Both paths must be kept in sync.
