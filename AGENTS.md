...
- **`max_context_tokens` is per-model, NOT per-subscription.** Stored in `PerModelConfigs[model].MaxContext`, NOT in user_settings DB or config.Agent.MaxContextTokens. `/settings` writes only the current session's model via `UpdatePerModelConfig`, preserving existing `MaxOutputTokens`. Must NOT pass `max_context_tokens` to `ApplySettings` — that calls `ag.SetMaxContextTokens(n)` globally, contaminating all models without per-model override.
...
- **`listSubscriptions`/`getDefaultSubscription` RPC must merge `subscription_models` table into `PerModelConfigs`.** The server stores per-model configs in two places: `subscription_models` table (v35+, authoritative) and `per_model_configs` JSON field. Without merging, client-side `ResolveEffectiveMaxContext` (which only checks `PerModelConfigs`) misses values from the table.
...
- **`liveIterationBlocks` deduplicates tools by `Name + Label`.** When a tool transitions from `ActiveTools(done)` to `CompletedTools` across progress frames, both lists can temporarily contain it. The dedup set (`seen[key]`) prevents the same tool from rendering twice.
