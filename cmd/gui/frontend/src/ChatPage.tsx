...
export default function ChatPage({ onLogout }: ChatPageProps) {
  ...
  // After messages update from loadHistory, scroll to bottom THEN reveal content.
  // Content stays hidden (opacity:0 via loadingHistory=true) until scroll completes,
  // so the user never sees the flash-to-top.
  useEffect(() => {
    if (!scrollToBottomAfterHistoryRef.current) return
    scrollToBottomAfterHistoryRef.current = false
    // Double-rAF: first paint renders new virtual items, second measures them
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        loadingHistoryRef.current = false // Allow scrollToFn to execute
        const v = virtualizerRef.current
        if (v && v.options.count > 0) {
          v.scrollToIndex(v.options.count - 1, { align: 'end', behavior: 'instant' })
        }
        setLoadingHistory(false) // Reveal content at the bottom
    ...
    fetch('/api/history')
      ...
      .then((data) => {
        ...
        if (data.ok && data.messages) {
          ...
          // Keep loadingHistory=true (content hidden via opacity:0).
          // The useEffect on [messages] will scroll to bottom then reveal.
    ...
    >
      ...
      <ChatSidebar
        ...
        onSwitchChat={(chatID: string) => {
            ...
            // Don't clear messages here — let loadHistory replace them atomically
            // to prevent flash-to-top during session switch.
            resetProgress()
            setTodos([])
            setSubAgents([])
            turnDoneRef.current = true
      ...
      >
        ...
        {/* Virtualized message list — hidden during history load, revealed after scroll-to-bottom */}
        {turns.length > 0 && (
          <div
            style={{
                    ...
                    >
                      <div data-msg-id={turn.message.id}>
                        ...
                        <div className="rounded-xl px-4 py-3 bg-blue-600 text-white markdown-body text-sm relative ml-auto user-msg"
                             style={{ maxWidth: '80%', width: 'fit-content', overflowWrap: 'break-word', wordBreak: 'break-word', userSelect: 'text' }}
                             onContextMenu={(e) => {
      ...
      {/* AskUser panel — inline above input bar */}
      {askUser && (
