...
func newTestBackendWithSettings(t *testing.T) (agent.RPCHandlerBackend, *agent.Agent, *sqlite.UserSettingsService) {
	t.Helper()
...
// Compile-time check: fakeBackend implements agent.RPCHandlerBackend.
var _ agent.RPCHandlerBackend = fakeBackend{}
...
func TestApplyRuntimeSetting_UpdatesConfig(t *testing.T) {
	cfg := newTestConfig()
	var backend agent.RPCHandlerBackend // nil is fine — we only test cfg mutation
...
func TestApplyRuntimeSetting_WarnsOnUnknownKey(t *testing.T) {
	cfg := newTestConfig()
	var backend agent.RPCHandlerBackend
